...
<div class="add-detail mt-10">
    <div class="w-50 float-left mt-10">
        <h1 class="m-0 p-0">Purchase Order</h1>
        <p class="m-0 pt-5 text-bold w-100">Purchase Order <span class="gray-color">{{$order->order_number}}</span></p>
        <p class="m-0 pt-5 text-bold w-100">Order Date - <span class="gray-color">{{\Carbon\Carbon::parse($order->created_at)->format('d-m-Y')}} {{\Carbon\Carbon::parse($order->created_at)->format('H:i A')}}</span></p>
    </div>
...
<div class="table-section bill-tbl w-100 mt-10">
    <table class="table w-100 mt-10">
        <tr>
            <th class="w-50">Vendor Information</th>
            <th class="w-50">Store Address</th>
        </tr>
        <tr>
            <td>
                <div class="box-text">
                    <p>Name: {{$order->supplier->name??""}},</p>
...
<div class="table-section bill-tbl w-100 mt-10">
    <table class="table w-100 mt-10">
        <tr>
            ...
            <th class="w-50">Ordered Qty</th>
            <th class="w-50">Received Qty</th>
            <th class="w-50">Due</th>
        </tr>
        ...
        <tr>
            <th>Totals</th>
            <th>{{$order->orderItems()->sum('ordered')}}</th>
            <th>{{$order->orderItems()->sum('received')}}</th>
            <th>{{$order->orderItems()->sum('outstanding')}}</th>
        </tr>
{{--        <tr>--}}
{{--            <td colspan="4">--}}
